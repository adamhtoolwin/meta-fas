...
def main(weights_file, writer, lr=0.005, maml_lr=0.01, iterations=1000, ways=5, shots=1, tps=32, fas=5, device=torch.device("cpu"),
    ...
    mnist_val = l2l.data.MetaDataset(MNIST(download_location,
                                             train=False,
                                             download=True,
                                             transform=transformations))

    val_tasks = l2l.data.TaskDataset(mnist_val,
                                       task_transforms=[
                                            l2l.data.transforms.NWays(mnist_val, ways),
                                            l2l.data.transforms.KShots(mnist_val, 2*shots),
                                            l2l.data.transforms.LoadData(mnist_val),
                                            l2l.data.transforms.RemapLabels(mnist_val),
                                            l2l.data.transforms.ConsecutiveLabels(mnist_val),
                                       ],
                                       num_tasks=1000)
    ...
    meta_model.load_state_dict(torch.load(weights_file))
...
if __name__ == '__main__':
    ...
    weights_file = configs['weights']

    main(weights_file=weights_file,
         writer=writer,
         lr=configs['lr'],
